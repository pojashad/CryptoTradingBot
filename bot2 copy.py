...
for index, row in df.iterrows():
    ...
    if dollarWallet > 0:
        if (
            row["Close"] < row["Closing EMA_500"]
            and row["Close"] > previousPrice
            and ETHs == 0
        ):
            print("--Buying--")
        ...
        # Sell when the closing price is higher than the buyPrice(buying price + comission) + profit margin. Securing that the buying & Sell commission are returned and a profit margin
            ...
            buyPrice * vinstMarginal < (row["Close"] - (row["Close"] * commission))
            and row["Close"] < previousPrice
            and ETHs > 0
        ):
            print("--Selling--")
            ...
            dollarWallet = dollarWallet + (
                ETHs * (row["Close"]) - (ETHs * (row["Close"]) * commission)
            )
...
df["BuyOrder"].plot(x=df.index, y=df.Close, c="blue", marker="^", label="Bought")
df["SellOrder"].plot(x=df.index, y=df.Close, c="red", marker="^", label="Sold")
df["TransactionPoint"].plot(
    x=df.index, y=df.Close, c="black", marker="^", label="Transaction"
)
